...
"""
    ...
    _TMP = [
			0, _BIT_POSITION_8, _BIT_POSITION_1, _BIT_POSITION_2, 
			_BIT_POSITION_3, _BIT_POSITION_4, _BIT_POSITION_5, 
    ...
    ################################################################################################
    # Similar with getNetworkISO but asciiIso = tpdu + self.getRawIso()
    def getNetworkISOwithTPDU(self, tpdu, bigEndian=True):
        """Method that add TPDU function
		    asciiIso = tpdu + self.getRawIso()
            # The lenght of TPDU is variable, in this case is lenght 12 ( text: ISO + internal routing code: 000000000 ).
            # The result is something like
            # asciiISO with TPDU
            RISO00000000008002038000001c0000000000013305813305816061501000000001000000000000000
            #  asciiISO without TPDU
            F08002038000001c0000000000013305813305816061501000000001000000000000000
            # Thanks to: Daniel 
            @param: tpdu -> to attach a TPDU string
            @param: bigEndian (True|False) -> if you want that the size be represented in this way.
            @return: size + ASCII ISO8583 package ready to go to the network!
            @raise: InvalidMTI Exception
        """
